...
const DraggableComponent: React.FC<DraggableComponentProps> = ({ component }) => {
  ...
  useEffect(() => {
    if (component.icon) {
      // Create a canvas with both image and text (no background, no shadow)
      const canvas = document.createElement('canvas');
      const ctx = canvas.getContext('2d');
      if (!ctx) {
        // Fallback to transparent gif if canvas fails
        const fallbackImg = new Image();
        fallbackImg.src = 'data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==';
        fallbackImg.onload = () => preview(fallbackImg);
        return;
      }
      ...
      img.onload = () => {
        ...
        const imgSize = 32;
        ctx.drawImage(img, (canvas.width - imgSize) / 2, 5, imgSize, imgSize);
        
        // Draw text below image (no shadow, simple black text)
        ctx.fillStyle = 'black';
        ...
        if (text.length > 12) {
          // Truncate long text
          ctx.fillText(text.substring(0, 9) + '...', canvas.width / 2, 42);
        } else {
        ...
        canvas.toBlob((blob) => {
          if (blob) {
            ...
            previewImg.onload = () => preview(previewImg);
      ...
      img.onerror = () => {
        // Fallback to transparent gif if image fails to load
        const fallbackImg = new Image();
        fallbackImg.src = 'data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==';
        fallbackImg.onload = () => preview(fallbackImg);
      };
      ...
      // Fallback to transparent gif for Lucide icons
      const img = new Image();
      img.src = 'data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==';
      img.onload = () => preview(img);
    }
  }, [preview, component.icon, component.name]);
