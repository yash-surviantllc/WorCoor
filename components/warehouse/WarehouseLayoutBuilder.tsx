...
function App({ initialOrgUnit = null, initialLayout = null }: AppProps) {
  ...
  const saveToUndoStack = useCallback((items: any[]) => {
  ...
  const getExistingLocationIds = useCallback((itemId: string) => {
    ...
    if (item.compartmentContents) {
      ...
      const compartmentLocationIds = Object.values(item.compartmentContents)
        ...
        .flatMap((content: any) => {
          ...
          if ((content as any).isMultiLocation && (content as any).locationIds) {

            return (content as any).locationIds;

          }

          // Handle single location ID (horizontal storage racks)

          return (content as any).locationId || (content as any).uniqueId;
  ...
  const handleLocationIdSelect = useCallback((data: any) => {
    ...
    if (data.isMultiple && item.type === 'vertical_sku_holder') {
      ...
      const resolvedMappings = levelLocationMappings.length > 0
        ...
        : locationIds.map((locId: any, index: number) => ({
      ...
      const resolvedLevelIds = levelIds.length > 0
        ...
        : resolvedMappings.map((mapping: any) => (mapping as any).levelId);



      const resolvedLocationIds = resolvedMappings.map((mapping: any) => (mapping as any).locationId);
    ...
    <DndProvider backend={HTML5Backend}>
      ...
      <div className="app">
        ...
        {showMainDashboard ? (
          ...
          <>
            ...
            <TopNavbar
              ...
              selectedOrgUnit={selectedOrgUnit as any}
              ...
              locationTags={locationTags as any[]}
            ...
            <div className="main-content">
              ...
              {(() => { console.log('WarehouseCanvas - warehouseItems:', warehouseItems); return null; })()}

              {(() => { console.log('WarehouseCanvas - warehouseItems length:', warehouseItems.length); return null; })()}

              {(() => { console.log('WarehouseCanvas - warehouseItems sample:', warehouseItems[0]); return null; })()}
        ...
        <MultiLocationSelector
          ...
          existingLocationIds={(pendingSkuRequest ? getExistingLocationIds(pendingSkuRequest.itemId) : []) as any}
