...
import { Eye, Plus, ShoppingCart, Search, Edit, Trash2, CheckCircle, Clock } from "lucide-react"
...
export default function InventorySoPage() {
    ...
    const [viewMode, setViewMode] = useState<"list" | "form" | "blockQty" | "pickList" | "history">("list");
        ...
        const fetchSkus = async () => {
            try {
                const response = await apiService.post({
                    ...
                    "page": 0,
                        includeFields: [
                            "id", "name", "code", "unitId", "totalQty",
                            "blockedQty", "wastageQty", "consumeQty", "dispatchQty"
                        ]
                    },
                    isAuth: true
                });
    ...
    // ########## Block Qty ########## //
    const handleBlockInventory = (so: any) => {
        setSelectedSo(so);
        setViewMode("blockQty"); // Switch to Block Inventory view
    };

    const blockInventoryClose = async () => {
        setSelectedSo(null);
        setViewMode("list");
        refreshList();
    };
    ...
    const handleOpenBlockInventoryConfrimDialog = (item: any, index: number) => {
        ...
        const prefillQty = availableQty > item.qty ? item.qty : availableQty;
    ...
    const handleConfirmBlock = async () => {
        ...
        if (modalQty > selectedSkuItem?.qty) {
        ...
        try {
            ...
            if (response.data.status === "OK") {
                notification.success(response.data.message);
                selectedSo.items[blockIndex]['blockedQty'] = modalQty;
                setBlockIndex(-1);
                setModalQty(0);
            } else {
                notification.error(response.data.message);
    ...
    // ########## Generate Pick List ########## //
    const handleGeneratePickList = async (so: any) => {
        try {
            const response = await apiService.post({
            ...
            // else if (response.data?.status === "NOT_ACCEPTABLE" && response.data?.code === 406) {
            // }
    ...
    const generatePickListClose = async () => {
        setSelectedSo(null);
        setViewMode("list");
        refreshList();
    };
    ...
    // ########## View History ########## //
    const handleViewhistory = async (so: any) => {
        try {
            const response = await apiService.get({
                path: `${api_url.worCoorService.inventory.soPickList.getPickList}/${so.id}`,
                isAuth: true
            });
            setViewPickList(response.data?.data?.items || null);
        } catch (error) {
            ...
            {viewMode === "list" && (
                <>
                    <div>
                        ...
                        <div className="rounded-2xl border-0 sm:border border-slate-200  backdrop-blur-sm text-card-foreground shadow-soft hover:shadow-medium transition-all duration-300 dark:border-slate-700 dark:bg-slate-800/80 p-0 sm:p-6">
                                    ...
                                    <Table className="table-auto">
                                        <TableHeader className="bg-gray-100 text-black dark:bg-slate-950 dark:hover:bg-slate-950">
                                            <TableRow>
                                                ...
                                                <TableHead className="text-black font-semibold whitespace-nowrap min-w-[100px]">SKU Items</TableHead>
                                                <TableHead className="text-black font-semibold whitespace-nowrap min-w-[150px]">Fulfillment (Blocked)</TableHead>
                                                <TableHead className="text-black font-semibold whitespace-nowrap min-w-[100px]">SO Status</TableHead>
                                                <TableHead className="text-black font-semibold whitespace-nowrap min-w-[100px]">History</TableHead>
                                                        ...
                                                        <TableRow key={s.id} onDoubleClick={() => handleSoRowDoubleClick(s)} className="bg-muted/30 cursor-pointer">
                                                            ...
                                                            <TableCell className="p-4 md:p-6">
                                                                ...
                                                                <div title={`${totalBlocked} / ${totalQty} blocked`}>
                                                                    <Progress value={Number(fulfillmentValue)}
                                                                        className={`w-xl h-2 
                                                                            ${fulfillmentValue < 50 ? "bg-red-200 [&>div]:bg-red-500" : ""}
                                                                            ${fulfillmentValue >= 50 && fulfillmentValue < 80 ? "bg-yellow-200 [&>div]:bg-yellow-500" : ""}
                                                                            ${fulfillmentValue >= 80 ? "bg-green-200 [&>div]:bg-green-500" : ""}
                                                                        `}
                                                                    />
                                                                </div>
                                                            </TableCell>
                                                            <TableCell className="p-4 md:p-6">
                                                                {getSoStatusBadge(s.status)}
                                                            </TableCell>
                                                            <TableCell className="p-4 md:p-6">
                                                                {s.status === 1 || s.status === 2 ? (
                                                                    <div className="flex text-gray-400 cursor-not-allowed">
                                                            ...
                                                            <TableCell className="p-4 md:p-6 flex items-center justify-center">
                                                                {[1,2,3].includes(s.status) && (
                                                                    <Button variant="outline" onClick={() => handleBlockInventory(s)}
                                                                        className="border-gray-300/60 bg-gray-200/40 hover:bg-gray-200/40 text-sm"
                                                                    > Block/Unblock
                                                                    </Button>
                                                                )}
                                                                {s.status == 3 && (
                                                                    <Button variant="outline" onClick={() => handleGeneratePickList(s)}
                                                                        className="border-gray-300/60 bg-gray-200/40 hover:bg-gray-200/40 text-sm ml-2"
                                                                    > Pick List
                                                                    </Button>
                                                                )}
                                                                {s.status == 6 && (
                                                                    <Button variant="outline" onClick={() => handleDispatched(s)}
                                                                        className="border-gray-300/60 bg-gray-200/40 hover:bg-gray-200/40 text-sm"
                                                                    > Dispatch
                                                                    </Button>
                                                                )}
                                            ...
                                            <Breadcrumb className="text-md">
                                                <BreadcrumbList>
                                                    ...
                                                    <BreadcrumbItem className="cursor-pointer">
                                                        <BreadcrumbLink asChild>
                                                            <div onClick={blockInventoryClose}>Sales Order Management</div>
                                                        </BreadcrumbLink>
                                                    </BreadcrumbItem>
                                                    <BreadcrumbSeparator />
                                                    <BreadcrumbItem>
                                                        <BreadcrumbPage>Block Inventory</BreadcrumbPage>
                                                    </BreadcrumbItem>
                                                </BreadcrumbList>
                                            </Breadcrumb>
                                        </nav>
                                ...
                                <div className="h-full flex-grow overflow-auto">
                                    <Table className="table-auto w-full">
                                        <TableHeader className="text-black dark:bg-slate-950">
                                            <TableRow>
                                                <TableHead className="font-semibold px-4 mix-w-[50px]">#</TableHead>
                                        ...
                                        <TableBody>
                                            {selectedSo.items.map((item: any, index: number) => (
                                                <TableRow key={item.skuId}
                                                    className="[&>td:nth-child(-n+3)]:bg-neutral-100 dark:[&>td:nth-child(-n+3)]:bg-slate-600"
                                                >
                                                    ...
                                                    {item.blockedQty != item.qty && (
                                                        <Button variant="outline" onClick={() => handleOpenBlockInventoryConfrimDialog(item, index)}
                                                            className="w-[90px] border-amber-300/40 bg-amber-400/40 hover:bg-amber-400/40 text-sm mr-2"
                                                        > Block
                                                        </Button>
                                                    )}
                                                    {item.blockedQty > 0 && (
                                                        <Button variant="outline" onClick={() => handleOpenUnblockInventoryConfrimDialog(item, index)}
            ...
            {viewMode === "pickList" && (
                <div className="h-full relative flex flex-col overflow-hidden">
                            ...
                            <nav className="flex-1">
                                <Breadcrumb className="text-md">
                                    <BreadcrumbList>
                                        ...
                                        <BreadcrumbItem className="cursor-pointer">
                                            <BreadcrumbLink asChild>
                                                <div onClick={generatePickListClose}> Sales Order Management </div>
                                            </BreadcrumbLink>
                                        </BreadcrumbItem>
                                        <BreadcrumbSeparator />
                                        <BreadcrumbItem>
                                    ...
                                    >
                                        ...
                                        <TableCell className="p-4">
                                            ...
                                            <Button variant="outline"
                                                className="border-slate-100/40 bg-slate-400/40 hover:bg-slate-400/40 font-medium rounded-full text-sm"
                    ...
                    <div className="footer flex items-center justify-end border-t border-slate-200/60 pt-5">
                        <Button
                            disabled={!isAllSkuAssigned || isGeneratingPickList}
                            onClick={() => setIsGenerateConfirmOpen(true)}
                            className="bg-darkblue text-white hover:bg-darkblue/90 rounded-lg px-5 mr-2"
                        >
                            {isGeneratingPickList ? "Generating.." : "Generate"}
                        </Button>
                        <Button type="button" variant="outline" onClick={generatePickListClose}>
                            Cancel
                        </Button>
                    </div>
                </div>
                        ...
                        <Table className="table-auto w-full">
                            <TableHeader className="text-black dark:bg-slate-950">
                                <TableRow>
                                    ...
                                    <TableHead className="font-semibold px-4 min-w-[100px]">SKU Qty</TableHead>
                                    <TableHead className="font-semibold px-4 min-w-[100px]">Blocked Qty</TableHead>
                                    <TableHead className="font-semibold px-4 min-w-[100px]">View Picklist</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {selectedSo.items.map((item: any, index: number) => (
                                    ...
                                    >
                                        ...
                                        <TableCell>{item.blockedQty ? item.blockedQty : "0"}</TableCell>
                                        <TableCell className="p-4">
                                            {[4, 5, 6, 7].includes(selectedSo.status) ? (
                                                <Button className="bg-blue-100 hover:bg-blue-200 text-blue-900 rounded-full font-medium text-sm p-2 px-4"
                ...
                <DialogContent onInteractOutside={(e) => e.preventDefault()} onEscapeKeyDown={() => setIsBlockConfrimDialogOpen(false)}
                    ...
                    <div className="h-full flex-grow-1 overflow-y-auto px-2 md:px-6 space-y-3 pt-6 mb-4">
                        ...
                        <p className="text-sm text-neutral-500">
                            Are you sure you want to block this product quantity?
                            <br />
                            ...
                            <TableBody>
                                {(viewPickList ?? [])
                                    ...
                                    .map((pl: any, idx: number) => (
                                        <TableRow key={pl.inventoryId || idx}>
                                            <TableCell className="p-4 md:p-4">{formatDateOnly(pl.date)}</TableCell>
                                            <TableCell className="p-4 md:p-4">{getLocationTagNameById(pl.locationId)}</TableCell>
                                            <TableCell className="p-4 md:p-4">{pl.qty}</TableCell>
