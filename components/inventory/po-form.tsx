...
const skuItemSchema = z.object({
  ...
  date: z.string().min(1, "Manufacturing date is required"),
...
export function PoForm({ initialData = {}, onSubmit, onCancel, vendors, units, POType, formMode, skus, skuUnits }: PoFormProps) {
  ...
  const selectedFrom = useWatch({ control: form.control, name: "from" });
  const selectedTo = useWatch({ control: form.control, name: "to" });
  ...
  const hasSKUItems = watchSkuItems?.length > 0;
  ...
  return (
    <div className="h-100vh flex flex-col">
      <div className="flex items-start justify-start flex-col relative">
        ...
        <div className="mb-2">
          <PageHeader title={`Create ${POType} Purchase Order`} />
        </div>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit((formData) => {
            const payload = {
              type: POType === "Internal" ? 1 : 2,
              poNumber: formData.poNumber,
              from: formData.from,
              to: formData.to,
              date: new Date(formData.poDate).toISOString(),
          ...
          className="flex flex-col flex-grow overflow-hidden space-y-4">
          {/* form fields */}
          <div className={`${
              fields.length === 0
                ? "h-auto"
            ...
            <div className="border border-dashed bg-gray-100 rounded-xl p-6 pb-8 mb-6 gap-4 space-y-2">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="grid gap-2">
                  <FormField control={form.control} name="poNumber"
                    render={({ field }) => (
                      <FormItem>
                ...
                <div className="grid gap-2">
                  <FormField control={form.control} name="poDate"
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-sm font-medium leading-none ">PO Date <span className="text-destructive">*</span></FormLabel>
                        <FormControl className="w-full flex">
                          <Input className="!w-full h-12 rounded-md border border-input bg-white/100 dark:bg-slate-800/80 dark:border-slate-700"
                            id="po-date-time" type="date"
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="grid gap-2">
                  {POType === "Internal" && (
                    <FormField control={form.control} name="from"
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-sm font-medium leading-none">From <span className="text-destructive">*</span></FormLabel>
                          <Select disabled={formMode === "edit"} onValueChange={field.onChange} value={field.value}>
                            <FormControl>
                        ...
                        <FormItem>
                          ...
                          <Select disabled={formMode === "edit"} onValueChange={field.onChange} defaultValue={field.value}>
                            ...
                            <SelectContent className="min-w-[var(--radix-select-trigger-width)]">
                              {vendors.length === 0 ? (
                                <div className="p-2 text-sm text-muted-foreground">No vendors found</div>
                              ) : (
                                vendors.map((v) => (
                ...
                <div className="grid gap-2">
                  <FormField control={form.control} name="to"
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-sm font-medium leading-none">To <span className="text-destructive">*</span></FormLabel>
                  ...
                  <Table className="table-auto  border-none shadow-none border-separate border-spacing-y-4 bg-none">
                    <TableHeader className="text-black dark:bg-slate-950">
                      <TableRow>
                        ...
                        <TableHead className="text-black font-semibold whitespace-nowrap px-4 min-w-[100px]">Manufacturing Date</TableHead>
                      ...
                      {fields.map((field, index) => {
                        ...
                        return isEditing ? null : (
                          <TableRow key={field.id} className="my-3 bg-gray-50 shadow-soft hover:bg-slate-100/40 rounded-xl">
                            ...
                            <TableCell className="p-4">{sku?.date ? sku?.date : "-"}</TableCell>
                    ...
                    <div className="p-4">
                      <CardContent className="w-full pt-4">
                        <>
                          ...
                          {/* Unit Price and Manufacturing Date */}
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                            ...
                            />
                            <FormField control={form.control} name={`skuItems.${index}.date`}
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel className="text-sm font-medium leading-none ">Manufacturing Date <span className="text-destructive">*</span></FormLabel>
                                  <FormControl className="w-full flex">
                                    <Input className="!w-full h-12 rounded-md border border-input bg-white/100 dark:bg-slate-800/80 dark:border-slate-700"
                                      type="date" {...field}
                                    />
                      ...
                      <div className="w-full flex items-center mb-4">
                        <div className="h-full w-full pr-4">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                            ...
                            />
                            <FormField control={skuDraftForm.control} name="date"
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel>Manufacturing Date <span className="text-destructive">*</span></FormLabel>
                                  <FormControl>
                                    <Input type="date" className="!w-full h-12 rounded-md border border-input bg-white/100 dark:bg-slate-800/80 dark:border-slate-700" placeholder="Date" {...field} />
                                  </FormControl>
                                  <FormMessage />
                                </FormItem>
                              )}
